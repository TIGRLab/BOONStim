...
process dilate_roi_mask{

    label 'connectome'
    input:
    tuple val(sub), val(hemi), path(roi_mask), path(midthick)

    output:
    tuple val(sub), val(hemi), path("${sub}.${hemi}.roi_mask_dilated.shape.gii"), emit: roi_mask
    ...
    wb_command -metric-dilate \
                ...
                !{midthick} \
                6 \
                !{sub}.!{hemi}.roi_mask_dilated.shape.gii
    '''
}
...
process make_symmetric_dscalar{
    ...
    output:
    tuple val(sub), path("${sub}.roi_mask_symmetric.dscalar.nii"), emit: roi_mask
    ...
    wb_command -metric-math \
                ...
                -var x !{left_mask} \
                -var y !{right_mask} \
                !{sub}.L.roi_symmetric.shape.gii

    wb_command -metric-math \
                "(x+y) > 0" \
                -var x !{right_mask} \
                -var y !{left_mask} \
                !{sub}.R.roi_symmetric.shape.gii

    wb_command -cifti-create-dense-scalar \
                !{sub}.roi_mask_symmetric.dscalar.nii \
                -left-metric !{sub}.L.roi_symmetric.shape.gii \
                -right-metric !{sub}.R.roi_symmetric.shape.gii
    '''
}
...
workflow mask_wf {
    ...
    main:
        ...
        dilate_roi_input = binarize_mask.out.bin_mask
                                        .combine(cifti, by:0)
                                        .map{s,h,b,c->[
                                                        s,h,b,
        ...
        dilate_roi_mask(dilate_roi_input)

        // Make ROI symmetric mask
        make_symmetric_input = dilate_roi_mask.out.roi_mask
                                            .groupTuple(by: 0, size: 2, sort: {it})
                                            .map{ s,h,m -> [s, m[0], m[1]] }
        make_symmetric_dscalar(make_symmetric_input)

    emit:
        mask = make_symmetric_dscalar.out.roi_mask
